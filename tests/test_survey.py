...
def test_survey_get_item(test_data_expected_survey):

    # Test access operator implementation
    assert isinstance(test_data_expected_survey[0], SurveyAnswers)


def test_survey_del_item(test_data_expected_survey):

    # Delete the first answer set
    del test_data_expected_survey[0]

    # Check that the right one was deleted
    assert test_data_expected_survey[0].student_name == 'Jane Doe'

    # Check that there is only one out of two answer sets left
    assert len(test_data_expected_survey) == 1


def test_survey_iterator(test_data_expected_survey):
    ...
    for answer, expected_name in zip(test_data_expected_survey, expected_names):
        assert answer.student_name == expected_name


def test_survey_filter_by_student_name(test_data_expected_survey):

    # Filter surveys
    filtered_survey_1 = test_data_expected_survey.filter_by_student_name('John Doe')
    filtered_survey_2 = test_data_expected_survey.filter_by_student_name('Jane Doe')
